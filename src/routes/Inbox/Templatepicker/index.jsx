...
const TemplatePicker = ({ contact, onClose, setMessages, refreshContacts }) => {
  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-br from-[#1A1A1A] to-[#252525] z-10 flex flex-col animate-slideInRight">
      {/* Header */}
      <div className="bg-gradient-to-r from-[#2A2A2A] to-[#1F1F1F] border-b border-[#16A249]/30 p-5 flex-shrink-0">
        <div className="flex items-center gap-3">
          <button
            onClick={onClose}
            className="p-2 rounded-lg bg-[#16A249]/10 hover:bg-[#16A249]/20 text-[#00FF7F] transition-all duration-300"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          {!selectedTemplate && (
            <button
              ...
              disabled={isLoading}
              className="p-2 rounded-lg bg-[#16A249]/10 hover:bg-[#16A249]/20 text-[#00FF7F] transition-all duration-300 disabled:opacity-50"
          ...
          <div className="mt-4 flex gap-2">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-[#1A1A1A] border border-[#16A249]/30 rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none focus:border-[#16A249] transition-all"
              />
            </div>
            {categories.length > 1 && (
              <div className="relative w-36">
                <select
                  ...
                  className="w-full px-3 py-2.5 bg-[#1A1A1A] border border-[#16A249]/30 rounded-lg text-white text-sm focus:outline-none focus:border-[#16A249] transition-all appearance-none cursor-pointer"
                >
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="text-sm text-[#00FF7F] hover:text-[#00FF7F] transition-colors"
            >
            ...
            {Object.keys(paramValues).length > 0 && (
              <div className="bg-[#2A2A2A] rounded-xl p-4 border border-[#16A249]/20 space-y-4">
                <p className="text-sm font-semibold text-white">Fill in variables</p>
                {variableGroups.header.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs text-[#00FF7F] font-medium">Header Variables</p>
                    {variableGroups.header.map(key => (
                      <div key={`header-${key}`}>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 bg-[#1A1A1A] border border-[#16A249]/30 rounded-lg text-white text-sm focus:outline-none focus:border-[#16A249] transition-all"
                        />
                      </div>
                    ))}
                  </div>
                ...
                {variableGroups.body.length > 0 && (
                  <div className="space-y-2">
                    {(variableGroups.header.length > 0 || variableGroups.buttons.length > 0) && (
                      <p className="text-xs text-[#00FF7F] font-medium">Body Variables</p>
                    )}
                ...
                {variableGroups.buttons.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs text-[#00FF7F] font-medium">Button Variables</p>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#16A249]/20 to-[#1C4F2A]/20 flex items-center justify-center mx-auto mb-4">
              <FileText className="w-10 h-10 text-[#00FF7F]" />
            </div>
              ...
              return (
                <div
                  ...
                  className={`bg-[#2A2A2A] rounded-xl border overflow-hidden transition-all duration-300 cursor-pointer
                    ${isSelecting
                      ? 'ring-2 ring-[#16A249] scale-[0.98] bg-[#16A249]/10 border-[#16A249]'
                      : isApproved
                        ? 'border-[#16A249]/20 hover:border-[#16A249]/50 hover:shadow-lg hover:shadow-[#16A249]/10 active:scale-[0.99]'
                ...
                >
                  ...
                  <div className="flex items-center justify-between p-4">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-3">
                      {isApproved ? (
                        <ChevronRight className={`w-4 h-4 text-[#00FF7F] transition-transform duration-300 ${isSelecting ? 'translate-x-1' : ''}`} />
                      ) : (
                        isExpanded ? (
                  ...
                  {isExpanded && !isApproved && (
                    <div className="px-4 pb-4 border-t border-[#16A249]/10 pt-3">
      ...
      {/* Footer */}
      <div className="bg-gradient-to-r from-[#2A2A2A] to-[#1F1F1F] border-t border-[#16A249]/30 p-4 flex-shrink-0">
        {selectedTemplate ? (
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 px-4 py-3 bg-gradient-to-r from-[#16A249] to-[#1C4F2A] text-white rounded-xl transition-all font-semibold hover:from-[#1EB956] hover:to-[#267A3D] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isSending ? (
                <>
